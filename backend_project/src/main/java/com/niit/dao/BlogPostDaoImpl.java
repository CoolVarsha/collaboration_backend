...
public class BlogPostDaoImpl implements BlogPostDao {
	...
	public List<BlogPost> getBlogs(boolean approved) {
		...
		return sessionFactory.getCurrentSession().createSQLQuery("select * From BLOGPOST_TABLE where approved=:approved").setParameter("approved",1).list();
		}
		else {
			return sessionFactory.getCurrentSession().createSQLQuery("select * From BLOGPOST_TABLE where approved=:approved").setParameter("approved",0).list();
		}
